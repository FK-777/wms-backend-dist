...
let HolidayService = class HolidayService {
    ...
    create(createHolidayDto) {
        const createHoliday = new this.holidayModel(createHolidayDto);
        return createHoliday.save();
    }
    findAll(where = {}) {
        return this.holidayModel.find(where).exec();
    }
    findOne(id) {
        return this.holidayModel.findById(id).exec();
    }
    update(id, updateHolidayDto) {
        return this.holidayModel.updateOne({ _id: id }, updateHolidayDto).exec();
    }
    remove(id) {
        return this.holidayModel.deleteOne({ _id: id }).exec();
    }
};
HolidayService = __decorate([
    common_1.Injectable(),
    __param(0, mongoose_1.InjectModel(holiday_entity_1.Holiday.name)),
